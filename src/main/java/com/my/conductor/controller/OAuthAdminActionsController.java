...
import com.my.conductor.dto.request.admin.ClientDeleteRequest;
import com.my.conductor.dto.request.admin.ClientRequest;
import com.my.conductor.dto.request.admin.ClientResetPasswordWithClienIdRequest;
import com.my.conductor.dto.request.admin.UserDeleteWithClienIdRequest;
import com.my.conductor.dto.request.admin.UserResetPasswordWithClienIdRequest;
import com.my.conductor.dto.request.admin.UserWithClientIDRequest;
import com.my.conductor.dto.response.AutogeneratedPasswordResponseDTO;
...
private Logger logger = LoggerFactory.getLogger(OAuthAdminActionsController.class.getSimpleName());
	...
	@Operation(summary = "Start Conductor Server if Auto Startup was Disabled or Shutdown was performed", description = "Returns the conductor server startup status", tags = { "conductor" })
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Conductor Server Start Triggered Successfully.", 
                content = @Content(schema = @Schema(implementation = BaseResponseDTO.class))) ,
        @ApiResponse(responseCode = "304", description = "Conductor Server is already started.", content = @Content()),
        @ApiResponse(responseCode = "500", description = "Internal Server Error - Returned when an unexpected error occurs on server side", content = @Content())})
	@PutMapping(value = "start-conductor-server", produces = "application/json")
	@ResponseBody
	public ResponseEntity<BaseResponseDTO>  startConductor() {
		...
		try {
			...
			if(!conductorRunnerThread.isRunning())
			{
				...
				conductorRunnerThread.configureArgs(args);
				conductorRunnerThread.setRunToTrue();
				conductorRunnerThread.start();
				

				response.setStatus(true);
				response.setMessage("Conductor Server Start Triggered Successfully.");
				return new ResponseEntity(response, HttpStatus.OK);
			...
			{
				response.setStatus(false);
				response.setMessage("Conductor Server is already started.");
